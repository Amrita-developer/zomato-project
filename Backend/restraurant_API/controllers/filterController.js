    ...
    getAllL: async (req, res) => {
        ...
        if (queryParams) {
            ...
            if (queryParams.cuisine) {
                //et cuisines=queryParams.cuisine
        ...
        //database.collection(collectionName).find(filterObj).limit(limit).skip(startIndex).sort(sortObj).toArray((err, result) => {
          locId1=queryParams.location
        database.collection(collectionName).find({ cuisine:filterObj,cost:costObj,location:locId1}).sort(sortObj).toArray((err, result) => {
            if (err) {
